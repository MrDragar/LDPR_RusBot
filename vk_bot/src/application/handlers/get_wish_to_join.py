...
async def get_wish(message: Message, user_service: IUserService, tg_bot: TgBot):
    ...
    if text == 'нет':
        await finish_registration(
            ...
            message.ctx_api, message.ctx_api.log_chat, message.state_dispenser,
            tg_bot
        )
    else:
        await message.state_dispenser.set(message.from_id,
